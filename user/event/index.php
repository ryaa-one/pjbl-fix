...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_event_id'])) {
    ...
    $execDeleteEvent = deleteEventWithRelations($koneksi, $deleteEventId, dirname(__DIR__, 2), $currentAdminId);
...
if ($statusMessage === "" && isset($_GET['status'])) {
    ...
    if ($_GET['status'] === 'deleted') {
        ...
        $statusMessage = "Perubahan event dikirim dan menunggu persetujuan admin.";
    }
}
?>
  ...
  <body>
    ...
    <div class="admin-layout">
      ...
      <main class="admin-content">
              ...
              <?php if ($execEvents && mysqli_num_rows($execEvents) > 0): ?>
                ...
                <?php while ($event = mysqli_fetch_assoc($execEvents)): ?>
                  ...
                  <tr>
                    ...
                    <td class="col-category">
                      <?php
                        $eventStatusLabel = ucfirst((string) $event['status']);
                        if (($event['latest_edit_status'] ?? '') === 'pending') {
                            $eventStatusLabel = 'Pending Edit';
                        }
                        if (! empty($event['rejection_reason'])) {
                            $eventStatusNotes[] = [
                                'event_title' => $event['title'],
                                'status' => ucfirst((string) $event['status']),
                                'note' => $event['rejection_reason'],
                            ];
                        }
                        if (! empty($event['edit_rejection_reason'])) {
                            $eventStatusNotes[] = [
                                'event_title' => $event['title'],
                                'status' => 'Edit Ditolak',
                                'note' => $event['edit_rejection_reason'],
                            ];
                        }
                      ?>
        ...
        <?php if (! empty($eventStatusNotes)): ?>
          <section class="pending-edit-summary" aria-labelledby="event-status-notes-title">
            <div class="pending-edit-summary__header">
              <h2 id="event-status-notes-title">Catatan Status Event</h2>
            </div>
            <div class="pending-edit-summary__table-wrap">
              <table class="events-table pending-edit-summary__table">
                ...
                <tbody>
                  <?php foreach ($eventStatusNotes as $index => $statusNote): ?>
                    <tr>
                      <td class="events-table__muted col-number"><?= $index + 1 ?></td>
                      <td><?= htmlspecialchars($statusNote['event_title']) ?></td>
                      <td><span class="category-badge"><?= htmlspecialchars($statusNote['status']) ?></span></td>
                      <td><?= htmlspecialchars($statusNote['note']) ?></td>
                    </tr>
                  <?php endforeach; ?>
                </tbody>
              </table>
      ...
      (function () {
        ...
        frame.addEventListener('load', function () {
          try {
            ...
            if (frameUrl.pathname.endsWith('/user/event/index.php') && frameUrl.searchParams.get('status') === 'updated') {
