...
function resolveEventImageSrc(string $imagePath): string
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            $supersededReason = 'Digantikan request edit terbaru.';
            $stmtSupersedeRequests = mysqli_prepare(
                $koneksi,
                "UPDATE event_update_requests SET status = 'rejected', rejection_reason = ? WHERE event_id = ? AND user_id = ? AND status = 'pending'"
            );
            if (! $stmtSupersedeRequests) {
                throw new RuntimeException('Request edit lama gagal diproses.');
            }
            mysqli_stmt_bind_param($stmtSupersedeRequests, 'sii', $supersededReason, $eventId, $currentAdminId);
            if (! mysqli_stmt_execute($stmtSupersedeRequests)) {
                throw new RuntimeException('Request edit lama gagal diproses.');
            }
            mysqli_stmt_close($stmtSupersedeRequests);

            $stmtUpdateEvent = mysqli_prepare(
                $koneksi,
                "
                INSERT INTO event_update_requests (
                    event_id, user_id, title, description, category_id, city_id,
                    start_date, end_date, location, thumbnail, gallery_carousel, status
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
            "
            );

            if (! $stmtUpdateEvent) {
                throw new RuntimeException('Request edit gagal disimpan.');
            }
            ...
            <div class="admin-upload">
                ...
                <div class="admin-upload__preview-thumb">
                  <?php if ($formData['thumnail'] !== ''): ?>
                    <img src="<?= htmlspecialchars(resolveEventImageSrc($formData['thumnail'])) ?>" alt="Preview thumbnail event" id="thumbnail-preview">
                  <?php else: ?>
