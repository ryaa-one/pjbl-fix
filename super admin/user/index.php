...
include '../../process/getSuperAdminUsers.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_user_id'])) {
    ...
    if ($deleteUserId === (int) $_SESSION['user_id']) {
        ...
        $stmtDeleteUser = mysqli_prepare($koneksi, "DELETE FROM users WHERE id = ? AND level = 'user'");
        if ($stmtDeleteUser) {
            mysqli_stmt_bind_param($stmtDeleteUser, 'i', $deleteUserId);
            $execDeleteUser = mysqli_stmt_execute($stmtDeleteUser);
            $affectedRows = mysqli_stmt_affected_rows($stmtDeleteUser);
            mysqli_stmt_close($stmtDeleteUser);
        } else {
            $execDeleteUser = false;
            $affectedRows = 0;
        }

        if ($execDeleteUser && $affectedRows > 0) {
    ...
    <div class="admin-layout">
      ...
      <main class="admin-content">
        ...
        <form class="admin-toolbar" method="get" action="">
          ...
          <input type="hidden" name="filter_level" value="user" />
