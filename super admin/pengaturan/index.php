...
{
    $stmtUser = mysqli_prepare($koneksi, "SELECT id, name, email, level, profile_photo FROM users WHERE id = ? AND level = 'super_admin' LIMIT 1");
...
if (! $user) {
    $errors[] = "Data akun super admin tidak ditemukan.";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $user && ($_POST['form_type'] ?? '') === 'update_profile') {
    ...
    if (empty($errors)) {
        ...
        if (empty($errors)) {
            ...
            $sql .= " WHERE id = ? AND level = 'super_admin'";
            $types .= "i";
            $params[] = $userId;
...
<html lang="id">
  <head>
    ...
    <title>Pengaturan Akun Super Admin - EvenTura</title>
