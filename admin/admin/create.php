...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmtExistingAdmin = mysqli_prepare($koneksi, "SELECT id FROM users WHERE email = ? LIMIT 1");
        mysqli_stmt_bind_param($stmtExistingAdmin, 's', $formData['email']);
        ...
        if ($existingAdmin) {
            ...
            $stmtCreateAdmin = mysqli_prepare($koneksi, "INSERT INTO users (name, email, password, whatsapp, instagram, profile_photo, level) VALUES (?, ?, ?, '', '', '', 'user')");
            mysqli_stmt_bind_param($stmtCreateAdmin, 'sss', $formData['name'], $formData['email'], $passwordHashed);
            $execCreateAdmin = mysqli_stmt_execute($stmtCreateAdmin);
